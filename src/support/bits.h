...
static inline uint64_t sign_extend(uint64_t val, size_t bit_count) {
...
static inline double bits_to_double(uint64_t x) {
    double y;
    memcpy(&y, &x, sizeof(x));
    return y;
}

static inline float bits_to_float(uint32_t x) {
    float y;
    memcpy(&y, &x, sizeof(x));
    return y;
}
